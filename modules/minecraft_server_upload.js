...
module.exports = (root) => {
    console.log("正在載入 minecraft_server_upload...");
    ...
    function upload_map(map_name, map_data) {
        // 處理上傳的地圖數據
        console.log(`上傳地圖：${map_name}`);
        // 這裡可以添加實際的上傳邏輯，例如將地圖數據存儲到資料庫或文件系統中

        fs.mkdirSync(`${dbDir}/rippou-ripple-server/survival`, { recursive: true }, (err) => {
            if (err) {
                console.error('建立資料夾失敗', err);
                return res.status(500).send('伺服器錯誤');
            }
            fs.writeFileSync(`${dbDir}/rippou-ripple-server/survival/${map_name}.json`, JSON.stringify(map_data, null, 2), 'utf8', (err) => {
                if (err) {
                    console.error('上傳地圖時發生錯誤：', err.message);
                    return;
                }
                console.log(`地圖 ${map_name} 已成功上傳`);
                return res.status(200).send('收到地圖');
            });
        });
    }
